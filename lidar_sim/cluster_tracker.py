...
class ClusterTracker:
    def __init__(self, max_distance=0.5, min_samples=5, history_size=5):
        ...
        self.history_size = (
            history_size  # How many past frames to consider for matching
        )
    ...
    def _associate_clusters(self, current_centroids):
        ...
        for i in range(len(current_centroids)):
            if i not in used_indices:
                self.previous_centroids = np.vstack(
                    (self.previous_centroids, current_centroids[i])
                )
                self.current_ids = np.append(
                    self.current_ids, self.cluster_counter
                )  # Use np.append for arrays
                self.cluster_counter += 1
    ...
    def track(self, filtered_points):
        ...
        centroids = np.array(
            [
                filtered_points[filtered_points[:, 2] == i][:, :2].mean(axis=0)
